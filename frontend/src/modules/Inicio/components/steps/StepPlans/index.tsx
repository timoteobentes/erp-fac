...
// MOCK DE PLANOS (Pode vir de uma prop ou config externa se preferir)
const PLANS = [
  {
    id: 'mei',
    name: 'MEI',
          ...
          {PLANS.map((plan) => {
            ...
            return (
              <Card 
                ...
                onClick={() => onSelect({ id: plan.id, name: plan.name, price: plan.price })}
              >
                {isRecommended && (
                  <Chip 
                    label="MAIS POPULAR" 
                ...
                <CardContent className="text-center pt-8 pb-4">
                  ...
                  <Box className="my-4 flex justify-center items-baseline text-gray-800">
                    <Typography variant="h3" fontWeight="bold">{plan.price.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</Typography>
