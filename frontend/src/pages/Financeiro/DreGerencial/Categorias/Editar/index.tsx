...
import { atualizarCategoriaDreService, buscarCategoriaDrePorIdService } from "../../../../../modules/Financeiro/DreGerencial/services/categoriasDreService";
import {
  ATIVO_OPTIONS,
...
const EditarCategoriaDre: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);

  const { register, handleSubmit, formState: { errors, isSubmitting }, setValue, watch, reset } = useForm<CategoriaDreFormData>({
  ...
  const onSubmit = async (data: CategoriaDreFormData) => {
    try {
      if (!id) return;
      await atualizarCategoriaDreService(id, data);
      toast.success("Categoria DRE atualizada com sucesso");
      navigate("/financeiro/dre-gerencial/categorias");
    } catch (error: any) {
      toast.error(error.response?.data?.message || "Erro ao atualizar Categoria DRE");
    }
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <Typography variant="body2" color="#64748B">
              Registro <strong className="text-[#3C0473]">#{id?.padStart(4, "0")}</strong> - ajuste os dados da categoria.
            </Typography>
          </div>
        </div>
      </div>
        ...
        <Box sx={{ backgroundColor: "#FFFFFF", borderRadius: "24px", border: "1px solid #E2E8F0", boxShadow: "0 4px 24px rgba(0,0,0,0.02)", p: { xs: 3, md: 5 } }}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormControl fullWidth error={!!errors.grupo} sx={premiumInputStyles}>
              <InputLabel id="editar-categoria-dre-grupo-label">Grupo *</InputLabel>
              <Select labelId="editar-categoria-dre-grupo-label" value={grupoSelecionado} label="Grupo *" onChange={(event) => setValue("grupo", event.target.value, { shouldValidate: true })}>
            ...
            <FormControl fullWidth error={!!errors.tipo} sx={premiumInputStyles}>
              <InputLabel id="editar-categoria-dre-tipo-label">Tipo *</InputLabel>
              <Select labelId="editar-categoria-dre-tipo-label" value={tipoSelecionado} label="Tipo *" onChange={(event) => setValue("tipo", event.target.value as "DESPESA" | "RECEITA" | "TOTALIZADOR", { shouldValidate: true })}>
            ...
            <FormControl fullWidth error={!!errors.ativo} sx={premiumInputStyles}>
              <InputLabel id="editar-categoria-dre-ativo-label">Ativo *</InputLabel>
              <Select labelId="editar-categoria-dre-ativo-label" value={String(ativoSelecionado)} label="Ativo *" onChange={(event) => setValue("ativo", event.target.value === "true", { shouldValidate: true })}>
...
export default EditarCategoriaDre;
