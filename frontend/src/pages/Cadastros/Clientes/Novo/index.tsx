...
import { 
  Button, Box, TextField, MenuItem, IconButton, FormControlLabel, Checkbox,
...
const NovoCliente: React.FC = () => {
  ...
  const { 
    control, handleSubmit, watch, getValues, setValue, formState: { errors } 
    ...
    defaultValues: {
      ...
      situacao: "ativo",
      nome: "",
      vendedor_responsavel: user?.nome_empresa || "",
      enderecos: [{ 
        tipo: "comercial", cep: "", logradouro: "", numero: "", bairro: "", 
        cidade: "", uf: "", pais: "Brasil", principal: true
      }],
      contatos: [{
        tipo: "telefone", nome: "", valor: "", cargo: "", principal: true
      }],
      foto: [],
      anexos: [],
  ...
  useEffect(() => {
    if(consultaCNPJ) {
      setValue("razao_social", consultaCNPJ.company.name);
      setValue("nome", consultaCNPJ.company.name);
      setValue("nome_fantasia", consultaCNPJ.alias ? consultaCNPJ.alias : "");
      ...
      setValue("enderecos.0.logradouro", consultaCNPJ.address.street);
      setValue("enderecos.0.numero", consultaCNPJ.address.number);
      setValue("enderecos.0.bairro", consultaCNPJ.address.district);
      setValue("enderecos.0.cidade", consultaCNPJ.address.city);
      setValue("enderecos.0.uf", consultaCNPJ.address.state);
    }
  }, [consultaCNPJ]);
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <Typography variant="body2" color="#64748B">
                Preencha os dados abaixo para criar um novo registro na base.
            </Typography>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-10">
            ...
            <Box>
              <div className="flex justify-between items-center mb-6">
                ...
                <Button 
                  startIcon={<Add />} 
                  onClick={() => appendEnd({ tipo: 'comercial', cep: '', logradouro: '', numero: '', bairro: '', cidade: '', uf: '', pais: 'Brasil', principal: fieldsEnd.length === 0 })}
            ...
            <Box>
              <div className="flex justify-between items-center mb-6">
                ...
                <Button 
                  startIcon={<Add />} 
                  onClick={() => appendCont({ tipo: 'email', nome: '', valor: '', cargo: '', principal: fieldsCont.length === 0 })}
                  sx={{ textTransform: 'none', fontWeight: 600, color: '#5B21B6', backgroundColor: '#F3E8FF', '&:hover': { backgroundColor: '#E9D5FF' }, borderRadius: '8px' }}
                >
                   Adicionar
                </Button>
