...
export class MarcaRepository {
  async criar(dados: Marca, usuarioId: number): Promise<number> {
    const result = await pool.query(
      `INSERT INTO marcas (usuario_id, nome, ativa)
       VALUES ($1, $2, true)
       RETURNING id`,
  ...
  async listar(usuarioId: number, filtros: FiltrosMarca = {}, paginacao?: { limit: number; offset: number }) {
    let query = `
      SELECT id, nome, ativa, criado_em, atualizado_em
      FROM marcas
  ...
  async excluir(id: number, usuarioId: number): Promise<void> {
    await pool.query(
      `UPDATE marcas SET ativa = false, atualizado_em = NOW() WHERE id = $1 AND usuario_id = $2`,
      [id, usuarioId]
    );
  }
